...
# Task 3.3 - Scaling and Performance Tuning
# ----------
...
embeddings = np.array(data_minilm['embeddings'])
metadata = data_minilm['metadata']

norms = np.linalg.norm(embeddings, axis=1, keepdims=True)
embeddings = embeddings / norms

# use FAISS with partitioning
...
# Handle token limits in OpenAI context
...
def response_with_api(query, query_history, top_k=3):
    top_chunks = timed_function(weighted_query, query, query_history, top_k=top_k)
    context = truncate_context(overlap_handling(top_chunks), max_tokens=1000)
